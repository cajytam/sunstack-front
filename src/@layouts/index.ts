...
export const createLayouts = (userConfig: UserConfig): Plugin => {
  ...
  const localStorageNavbarType: 'sticky' | 'static' | null = localStorage.getItem(
    `${userConfig.app.title}-navBarType`,
  ) as 'sticky' | 'static' | null

  const localStorageContentLayoutNav: 'horizontal' | 'vertical' | null = localStorage.getItem(
    `${userConfig.app.title}-contentLayoutNav`,
  ) as 'horizontal' | 'vertical' | null
